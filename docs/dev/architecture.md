...
- **Staged implementation plan**: The intended Development Agent sequence is: (1) record support objects at production time for newly derived and already-known conclusions; (2) expose support verification APIs that can answer whether a Fact remains supported after one support path is invalidated; (3) implement recursive **Mark-Verify-Sweep** over the dependency graph; and only then (4) wire full triple removal through `RETEEngine`, `RETEStore`, and `BatchDispatcher`. Steps (1), (2), and (3) are implemented at the TMS-controller layer; step (4) is still pending. The support verification API surface is captured in [DR-023 JTMS Support Verification API Surface](decision-records/DR-023%20JTMS%20Support%20Verification%20API%20Surface.md), and the recursive retraction primitive is captured in [DR-024 TMSController Recursive Retraction](decision-records/DR-024%20TMSController%20Recursive%20Retraction.md). Until step (4) is wired, callers MUST treat `TMSController.retract_triple` as engine-internal: invoking it without coordinated alpha/beta network and `Agenda` cleanup will leave stale references in the engine.
